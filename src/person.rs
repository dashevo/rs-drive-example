...
impl Person {
    ...
    fn add_single(&self, drive: &Drive, contract: &Contract) {
        ...
        match document_type_result {
            Ok(document_type) => {
                match drive.worst_case_fee_for_document_type_with_name(contract, doument_type_name) {
                    ...
                    Err(e) => { println!("### ERROR! Could not get worst case fee from contract"); }
                }
            }
            Err(_) => {
...
fn person_rl(drive: &Drive, contract: &Contract, mut rl: &mut Editor<()>) -> bool {
...
pub fn person_loop(drive: &Drive, contract: &Contract, mut rl: &mut Editor<()>) -> bool {
